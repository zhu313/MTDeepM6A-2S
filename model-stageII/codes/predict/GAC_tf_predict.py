...
#    modelall.load_weights('modelall_w_GAC.h5')
    modelall.load_weights(modelpath)
    seqids = []
    allposs = []
    test = []
    ...
    test = np.array(test)
    y_pred = modelall.predict(test)
    outfile = sys.argv[3]
    output = open(outfile,'w')
    output.write("seqid\t    pos\t        probability\n")
    for i in range(len(allposs)):
        output.write('%5d\t  %5d\t%19.3f\n' % (seqids[i],allposs[i],y_pred[i,1]))
    output.close()
