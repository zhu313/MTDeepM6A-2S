...
def transferGAC(base_model):
    x = base_model.layers[-3].output
    output = Dense(2, activation='softmax',name='out1')(x)
...
if __name__ == "__main__":
    ...
    p_train_gac = readfa0('../../data/pos_train_hr.fasta')
    n_train_gac = readfa0('../../data/neg_train_hr.fasta')
    x_train_gac = list(chain.from_iterable(zip(p_train_gac, n_train_gac)))
#    p_train_gac = np.array(p_train_gac)
#    n_train_gac = np.array(n_train_gac)
#    x_train = np.vstack([p_train_gac, n_train_gac])
    x_train= np.array(x_train_gac)
    trainlabel = [[0,1],[1,0]]*len(p_train_gac)
#    trainlabel = [[0,1]]*len(p_train_gac) + [[1,0]]*len(n_train_gac)
    y_train_cls = np.array(trainlabel)

#    p_test_gac = readfa0('../pos_test_1.fasta')
#    n_test_gac = readfa0('../neg_test_balanced_1.fasta')
#    p_test_gac = np.array(p_test_gac)
#    n_test_gac = np.array(n_test_gac)
#    x_test = np.vstack([p_test_gac, n_test_gac])
#    testlabel = [[0,1]]*len(p_test_gac) + [[1,0]]*len(n_test_gac)
#    y_test_cls = np.array(testlabel)
    ###next for transfer learning###
...
#    tmp_all_index = list(kfold.split(x_train,y_train_cls[:,1]))
    ...
    base_model.load_weights('../../../models_stageI/codes/retrain/GAC_model.h5')
    modelall = transferGAC(base_model)
    modelall.fit(x_train,y_train_cls,epochs=64,batch_size=256)
    modelall.save_weights('./modelall_w_GAC.h5')
#    all_results = []
#    all_test_results = []
#    for epks in [16,32,64,128,256]:
#        for train_index,test_index in tmp_all_index:
#            train_X, train_y, val_X, val_y = x_train[train_index], y_train_cls[train_index], x_train[test_index], y_train_cls[test_index]
#            base_model  = LSTM_model()
#            base_model.load_weights('../../MTDeepM6A/codes/retrain/GAC_model.h5')
#            model = transferGAC(base_model)
#            model.fit(train_X,train_y,epochs=epks,batch_size=256)
#            val_pred = model.predict(val_X)
#            tmp_tn, tmp_fp, tmp_fn, tmp_tp, tmp_spe, tmp_sen, tmp_acc, tmp_mcc, tmp_auroc, tmp_auprc = my_score(val_pred,val_y)
#            all_results.append([epks,tmp_tn, tmp_fp, tmp_fn, tmp_tp, tmp_spe, tmp_sen, tmp_acc, tmp_mcc, tmp_auroc, tmp_auprc])
#        base_model  = LSTM_model()
#        base_model.load_weights('../../MTDeepM6A/codes/retrain/GAC_model.h5')
#        modelall = transferGAC(base_model)
#        modelall.fit(x_train,y_train_cls,epochs=epks,batch_size=256)
#        test_pred = modelall.predict(x_test)
#        test_tn, test_fp, test_fn, test_tp, test_spe, test_sen, test_acc, test_mcc, test_auroc, test_auprc = my_score(test_pred,y_test_cls)
#        all_test_results.append([epks,test_tn, test_fp, test_fn, test_tp, test_spe, test_sen, test_acc, test_mcc, test_auroc, test_auprc])
#    all_results = np.array(all_results)
#    all_test_results = np.array(all_test_results)
#    np.savetxt('tf_balance_diffEpochs_5.txt',all_results)
#    np.savetxt('tf_balance_diffEpochs_indep_3.txt',all_test_results)
