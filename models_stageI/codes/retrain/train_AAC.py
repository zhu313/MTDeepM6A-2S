...
if __name__ == "__main__":
    ...
    x_train= np.array(x_train_aac)
    y_train_reg = np.array(y_train_aac).reshape(-1,1)
    trainlabel = [[0,1],[1,0]]*len(p_train_aac)
    y_train_cls = np.array(trainlabel)
    mm = MinMaxScaler()
    y_train_reg = mm.fit_transform(y_train_reg)
    model = LSTM_model()
    model.fit(x_train,[y_train_cls,y_train_reg],epochs=60, batch_size=256, verbose=0)
    model.save_weights('./model_w_AAC.h5')
